...
```{r}
hotspots_VIC$fire_id_6km_mean = temp_fire_id
output_6km_mean = as.data.frame(hotspots_VIC) %>%
  select(`#obstime`, firepower, state, year, month, day, week, hour, hour_id, obs_id, fire_id_6km_mean)
temp = as.data.frame(st_coordinates(hotspots_VIC))
output_6km_mean$lon = temp$X
output_6km_mean$lat = temp$Y

write.csv(output_6km_mean, 'data/hotspots_6km_mean.csv', row.names = FALSE)
write.csv(center_move, 'data/fire_6km_mean.csv', row.names = FALSE)
rm(output_6km_mean)
```
