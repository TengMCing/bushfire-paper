...
# if the distance between two points is less than 9 km, create an edge between them
neighbors = (dist_matrix<9000) 
...
for (i in seq(2,max_hour)){
  ...
  # connect two points if they have less than 9000m distance
  neighbors = (dist_matrix<9000) 
...
```{r}
hotspots_VIC$fire_id_9km = temp_fire_id
output_9km = as.data.frame(hotspots_VIC) %>%
  select(`#obstime`, firepower, state, year, month, day, week, hour, hour_id, obs_id, fire_id_9km)
temp = as.data.frame(st_coordinates(hotspots_VIC))
output_9km$lon = temp$X
output_9km$lat = temp$Y

write.csv(output_9km, 'data/hotspots_9km.csv', row.names = FALSE)
write.csv(center_move, 'data/fire_9km.csv', row.names = FALSE)
rm(output_9km)
```
