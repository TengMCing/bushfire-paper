...
calc_max_r = function(X,Y,center_X,center_Y){
  ...
  max_r = max(as.numeric(st_distance(points, centeroid)))
  return(max_r)
}
...
group_centers[[1]] = as.data.frame(st_coordinates(first_hour)) %>%
  ...
  group_by(fire_id) %>%
  summarise(lon = mean(X), lat = mean(Y), max_r = calc_max_r(X,Y,mean(X),mean(Y)))
...
for (i in seq(2,max_hour)){
  ...
  # connect two points if they have less than 6000m distance or the max_r of the active fires
  if (nrow(active_group)>0){

    temp_matrix = matrix(0L, nrow = dim(dist_matrix)[1], ncol = dim(dist_matrix)[2])
    partial_1 = matrix(rep(active_group$max_r, dim(dist_matrix)[1]), nrow = dim(dist_matrix)[2], byrow = TRUE)
    partial_2 = matrix(rep(active_group$max_r, dim(dist_matrix)[2]), ncol = dim(dist_matrix)[2], byrow = FALSE)
    temp_matrix[(nrow(current_hour)+1):dim(dist_matrix)[1],] = partial_2
    temp_matrix[,(nrow(current_hour)+1):dim(dist_matrix)[2]] = partial_1

    neighbors = ((dist_matrix<6000) + (dist_matrix<temp_matrix))>0
  ...
  # now all the current hour points has a fire id, we can recompute the centers and max_r
...
```{r}
hotspots_VIC$fire_id_6km_max = temp_fire_id
output_6km_max = as.data.frame(hotspots_VIC) %>%
  select(`#obstime`, firepower, state, year, month, day, week, hour, hour_id, obs_id, fire_id_6km_max)
temp = as.data.frame(st_coordinates(hotspots_VIC))
output_6km_max$lon = temp$X
output_6km_max$lat = temp$Y

write.csv(output_6km_max, 'data/hotspots_6km_max.csv', row.names = FALSE)
write.csv(center_move, 'data/fire_6km_max.csv', row.names = FALSE)
rm(output_6km_max)
```
