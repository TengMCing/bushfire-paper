...
```{r}
hotspots_VIC$fire_id_3km_max = temp_fire_id
output_3km_max = as.data.frame(hotspots_VIC) %>%
  select(`#obstime`, firepower, state, year, month, day, week, hour, hour_id, obs_id, fire_id_3km_max)
temp = as.data.frame(st_coordinates(hotspots_VIC))
output_3km_max$lon = temp$X
output_3km_max$lat = temp$Y

write.csv(output_3km_max, 'data/hotspots_3km_max.csv', row.names = FALSE)
write.csv(center_move， 'data/fire_3km_max.csv', row.names = FALSE)
rm(output_3km_max)
```
