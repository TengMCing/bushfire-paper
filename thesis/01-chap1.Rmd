...
The statistical inference will be performed for testing the differences between human lighted bushfires and lightning-ignition bushfires.

## Modelling fire risk

The framework of fire risk modelling is similar to ignition method prediction. It will still be a classification problem, but a binary classification problem instead. The aim of this model is to predict the probability of bushfire ignition in a given grid at a given time. The grid will be designed to fit the shape of Victoria and with spatial resolution equal to 50km. The training data will be the hotspots data instead of bushfires history. Potential models will be the same as ignition method prediction.
