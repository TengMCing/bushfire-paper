...
The multinomial logit model, generalised additive model, random forest and XGboost are available in package `nnet` [@nnet], `mgcv` [@mgcv], `randomForest` [@R-rf] and `xgboost` [@xgboost] respectively. Besides, package `lime` [@lime] is used to perform feature selection and package `caret` [@R-caret] is used to control the training, hyperparameter tuning and candidate model selection process. 
