...
Our predictive model can produce the probability of the bushfire ignited by a certain type of sources $P(M|S,\mathcal{F})$, but it doesn't provide us with the risk of the bushfire ignited by that source $P(M,S|\mathcal{F})$, where $M$ is the method of the bushfire ignition, $M \in \{\text{lightning, arson, accident, burning off}\}$, $S$ is the bushfire is ignited and $\mathcal{F}$ is the supplementary information of the bushfire ignition, such as location, time, weather conditions, distribution of vegetation.

Knowing $P(M|S,\mathcal{F})$ is useful for bushfire investigation but is not particularly helpful for predicting the risk of the bushfire ignition. In general, decision makers want to know the risk of the bushfire ignited by a certain method $P(M,S|\mathcal{F})$ or the overall risk $P(S|\mathcal{F})$.
...
We will demonstrate how this works by using a simple method to estimate $P(S|\mathcal{F})$ and eventually yield $P(M,S|\mathcal{F})$.

If we only consider month $MON$ and location $L$ in $\mathcal{F}$, a reasonable estimator of
the probability of the bushfire ignited in a region $l$ in a certain month $mon$ is the relative frequency calculated from historical records:
