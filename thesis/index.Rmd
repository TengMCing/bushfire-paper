...
R, RStudio and Python are used for data analysis in this thesis. R packages used in this research include `tidyverse`, `sf`, `lubridate`, `rnaturalearth`, `here`, `raster`, `bomrang`, `geodist`, `progress`, `GGally`, `naniar`, `ggridges`, `ggthemes`, `caret`, `lime`, `nnet`, `randomForest`, `mgcv`, `fastDummies`, `xgboost`, `plyr` and `pROC`. Python packages used in this research include `tqdm` and `numpy`. Besides, this thesis is produced using the R package `bookdown`.
