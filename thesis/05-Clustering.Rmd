...
This algorithm can be divided into four steps: (1) Slice the temporal dimension, (2) Cluster hotspots locally, (3) Broadcast and updated the labels and (4) Compute ignition locations.
...
To illustrate this step, we need to introduce the first parameter used in our algorithm, $ActiveTime$. $ActiveTime$ was a parameter which controlled the size of the segments. A nature interpretation of this parameter was the time a fire could stay smouldering but undetectable by satellite before flaring up again. In @loboda2007reconstruction’s work, a similar parameter was used as the tolerance time for the missed observations due to cloud cover.

Given a certain value of $ActiveTime$ and the length of the time frame $T$, the algorithm would slice the timeline into several segments, $$\boldsymbol{S}_t = [max(1, t - ActiveTime), t], t = 1,2,...,T$$, where $T$ and $t$ have the same unit as $ActiveTime$. 

For example, if we have 48 hours of data and we set $ActiveTime = 24~hours$, the algorithm will produce 48 segments, $\boldsymbol{S}_1 = [1,1],\boldsymbol{S}_2 =[1,2],...,\boldsymbol{S}_{25}=[1,25],\boldsymbol{S}_{26}=[2,26],...,\boldsymbol{S}_{47} = [23,47],\boldsymbol{S}_{48} = [24,48]$.


### Cluster hotspots locally

In the previous step, the algorithm broke the temporal dimension. Thus, the local clustering for each segment only needed to address the hotspots spatially. This step involved the second parameter used in this algorithm, $AdjDist$. It represented the potential distance a fire could spread with respect to the temporal resolution of the data. For example, if $AdjDist = 3km$ and the temporal resolution of the data is 10-minute, then the potential speed of the bushfire is $3km/10min = 18km/h$. Similarly, in @ester1996density's work, they defined a radius-based parameter $\epsilon$ to connect neighbouring points.
...
### Broadcast and update the label


With local clustering results, the next step was to broadcast the clustering results from earlier segments to update the label iteratively. 

This step started from the second segment $\boldsymbol{S}_2$ till the segment $\boldsymbol{S}_T$. Given the segment $\boldsymbol{S}_t$, there were three substeps:
...
In this step, the algorithm combined the knowledge in different segments to produce the overall clustering result. By far, every hotspot has its membership label.

### Compute ignition locations

The earliest observed hotspot of a cluster was used as the ignition point. If there were multiple earliest hotspots within a cluster, the centroid of these points would be computed and used as the ignition point.

### Discussion of the algorithm

This section will reference to the conditions we discuss in the section [4.2][Outline of the algorithm]. In terms of condition (1), this algorithm didn't depend on the scaling of the temporal dimension, instead, it uses the parameter $ActiveTime$ to control the time frame. Condition (2) and (3) are embedded in the algorithm. The three substeps for label updating in step 4 is designed to meet condition (4). For condition (5), the memory complexity of this algorithm is $O(N)$, and the time complexity is $O(N^2)$. Although the time complexity of this algorithm is not as great as **DBSCAN**, it is still acceptable for the personal computer. Condition (7) will be justified in the Appendix about the Effects of parameter choices. 


## Results

The clustering results by applying our algorithm on the hotspots data in Victoria is shown in Figure \ref{fig:afterhotspot}. The corresponding three dimensional plot is given in Figure \ref{fig:tdafterhotspots}. From the distribution of ignition points (Figure \ref{fig:afterhotspot}), we do not observe too many overlapped points. This is consistent with the fact that the environment around the ignition point is often severely damaged and is less likely to be ignited again. Comparing to the hotspots data (Figure \ref{fig:rawhotspot}), ignition points can provide us with direct information of ignition locations and ignition time. This is crucial for our research on the cause of bushfires.
