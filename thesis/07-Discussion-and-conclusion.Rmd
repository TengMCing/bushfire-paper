...
We show the use of our predictive model in producing the fire risk map. However, a simple method is used to estimate the probability of bushfire ignited in a region. None of the changes of the distribution of vegetation and differences in the weather conditions have been considered. In the future work, we will develop another model to provide accurate prediction of the risk of the bushfire ignition $P(S|\mathcal{F})$. Combining the outcomes of two models, we will eventually produce the risk of bushfire ignited by different methods $P(M, S|\mathcal{F})$.

### Shiny app

General public and officers from fire departments may not have time and knowledge in running the code of our predictive model. An interactive web-based shiny application could be a more accessible way for people to be beneficial from our research. It could provide elegant interactive plots for exploratory data analysis and real-time prediction for decision making. A shiny app is under development by @shiny-app. The product will be published on the server of the Department of Econometrics and Business Statistics, Monash University.
